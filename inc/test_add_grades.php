...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	var_dump($dateMatch[0]);
	var_dump($dateMatch[1]);
	var_dump($dateMatch[2]);
	echo count($dateMatch);
	echo strlen($dateMatch[0]);
	echo strlen($dateMatch[1]);
	echo strlen($dateMatch[2]);
	echo checkdate($dateMatch[0],$dateMatch[1],$dateMatch[2]);

	$validation_conditions = empty($student_id) || empty($grade_id) || empty($date) || empty($title) || empty($description) || empty($max_points) || empty($earned_points) || count($dateMatch) != 3 || strlen($dateMatch[0]) != 2 ||
		strlen($dateMatch[1]) != 2 || strlen($dateMatch[2] != 4) || !checkdate($dateMatch[0],$dateMatch[1],$dateMatch[2]);

	if(!$validation_conditions){
			echo "Could not add grade...Oh no!";
		...
		if(add_grade($student_id,$grade_id,$date,$title,$description,$max_points,$earned_points)){
			...
			header('location:test_add_grades.php?status=thanks');
			exit;
		}else{
			$error_message = "Could not add grade";
		}
	}
}
...
if(isset($_GET['status']) && !empty($_GET['status']) && $_GET['status'] == 'thanks'){
	echo '<h1> New grade added! Thank you! </h1>';
	exit;
}
...
if(isset($error_message)){
	...
	echo '<h3> Add grade for student: </h3>';
}

?>


<form action="./test_add_grades.php" method="post">
	...
	<label for="grade_id"> Grade ID: <input type="text" name="grade_id" id="grade_id" placeholder="<?php 
